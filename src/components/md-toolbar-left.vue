<template>
    <div class="v-left-item">
        ...
        <button :disabled="!editable" type="button" v-if="toolbars.bold" @click="$clicks('bold')"
                ...
                :title="`${d_words.tl_bold} (ctrl+b)`"><span style="display:flex">粗体</span></button>
        <button :disabled="!editable" type="button" v-if="toolbars.italic" @click="$clicks('italic')"
                ...
                :title="`${d_words.tl_italic} (ctrl+i)`"><span style="display:flex">斜体</span></button>
        <div :class="{'selected': s_header_dropdown_open}" :disabled="!editable" type="button" v-if="toolbars.header" @mouseleave="$mouseleave_header_dropdown" @mouseenter="$mouseenter_header_dropdown"
                ...
                :title="`${d_words.tl_header} (ctrl+h)`"><span style="display:flex">标题</span>
        ...
        <button :disabled="!editable" type="button" v-if="toolbars.underline" @click="$clicks('underline')"
                ...
                :title="`${d_words.tl_underline} (ctrl+u)`" aria-hidden="true"><span style="display:flex">下划线</span></button>
        <button :disabled="!editable" type="button" v-if="toolbars.strikethrough" @click="$clicks('strikethrough')"
                ...
                :title="`${d_words.tl_strikethrough} (ctrl+shift+d)`" aria-hidden="true"><span style="display:flex">删除线</span></button>
        <button :disabled="!editable" type="button" v-if="toolbars.mark" @click="$clicks('mark')"
                ...
                aria-hidden="true"><span style="display:flex">标记</span></button>
        <button :disabled="!editable" type="button" v-if="toolbars.superscript" @click="$clicks('superscript')"
                ...
                aria-hidden="true" :title="`${d_words.tl_superscript} (ctrl+alt+s)`"><span style="display:flex">上角标</span></button>
        <button :disabled="!editable" type="button" v-if="toolbars.subscript" @click="$clicks('subscript')"
                class="op-icon fa fa-mavon-subscript"
                aria-hidden="true" :title="`${d_words.tl_subscript} (ctrl+shift+s)`"><span style="display:flex">下角标</span></button>
        <button :disabled="!editable" type="button" v-if="toolbars.alignleft" @click="$clicks('alignleft')"
                class="op-icon fa fa-mavon-align-left"
                aria-hidden="true" :title="`${d_words.tl_alignleft} (ctrl+l)`"><span style="display:flex">居左</span>
        </button>
        <button :disabled="!editable" type="button" v-if="toolbars.aligncenter" @click="$clicks('aligncenter')"
                ...
                aria-hidden="true" :title="`${d_words.tl_aligncenter} (ctrl+e)`"><span style="display:flex">居中</span></button>
        <button :disabled="!editable" type="button" v-if="toolbars.alignright" @click="$clicks('alignright')"
                ...
                aria-hidden="true" :title="`${d_words.tl_alignright} (ctrl+r)`"><span style="display:flex">居右</span></button>
        <span
        ...
        <button :disabled="!editable" type="button" v-if="toolbars.link" @click.stop="$toggle_imgLinkAdd('link')"
                ...
                :title="`${d_words.tl_link} (ctrl+l)`"><span style="display:flex">链接</span></button>
        <div :disabled="!editable" :class="{'selected': s_img_dropdown_open}" type="button" v-if="toolbars.imagelink" @mouseleave="$mouseleave_img_dropdown" @mouseenter="$mouseenter_img_dropdown"
                ...
                aria-hidden="true"><span style="display:flex">图片</span>
            <transition name="fade">
                <div  class="op-image popup-dropdown" :class="{'transition': transition}" v-show="s_img_dropdown_open" @mouseleave="$mouseleave_img_dropdown" @mouseenter="$mouseenter_img_dropdown">
                    <div  class="dropdown-item" ><span>{{d_words.tl_image}}</span></div>
        ...
        <!-- <button :disabled="!editable" type="button" v-if="toolbars.code" @click="$clicks('code')"
                ...
                :title="`${d_words.tl_code} (ctrl+alt+c)`"><span style="display:flex">代码块</span></button>
         -->
        <div :disabled="!editable" :class="{'selected': s_coder_dropdown_open}" type="button" v-if="toolbars.code" @mouseleave="$mouseleave_code_dropdown" @mouseenter="$mouseenter_code_dropdown"
                class="op-icon fa fa-mavon-code dropdown dropdown-wrapper"
                aria-hidden="true"><span style="display:flex">代码块</span>
            <transition name="fade">
                <div class="op-code popup-dropdown" style="overflow:auto;height:300px;width:100px;" :class="{'transition': transition}" v-show="s_code_dropdown_open" @mouseenter="$mouseenter_coder_dropdown" @mouseleave="$mouseleave_coder_dropdown">
                    <div title="CSS"  class="dropdown-item" @click.stop="$click_code('CSS')"><span>{{d_words.tl_CSS}}</span></div>
                    <div title="Java " class="dropdown-item" @click.stop="$click_code('Java')"><span>{{d_words.tl_Java}}</span></div>
                    <div title="Python " class="dropdown-item" @click.stop="$click_code('Python')"><span>{{d_words.tl_Python}}</span></div>
                    <div title="JavaScript " class="dropdown-item" @click.stop="$click_code('JavaScript')"><span>{{d_words.tl_JavaScript}}</span></div>
                    <div title="Bash " class="dropdown-item" @click.stop="$click_code('Bash')"><span>{{d_words.tl_Bash}}</span></div>
                    <div title="C++ " class="dropdown-item" @click.stop="$click_code('C++')"><span>{{d_words.tl_Csum}}</span></div>
                    <div title="C " class="dropdown-item" @click.stop="$click_code('C')"><span>{{d_words.tl_C}}</span></div>                    
                    <div title="C# " class="dropdown-item" @click.stop="$click_code('csharp')"><span>{{d_words.tl_Cshap}}</span></div>
                    <div title="Go " class="dropdown-item" @click.stop="$click_code('Go')"><span>{{d_words.tl_Go}}</span></div>
                    <div title="PHP " class="dropdown-item" @click.stop="$click_code('PHP')"><span>{{d_words.tl_PHP}}</span></div>
                    <div title="Perl " class="dropdown-item" @click.stop="$click_code('Perl')"><span>{{d_words.tl_Perl}}</span></div>
                    <div title="SQL " class="dropdown-item" @click.stop="$click_code('SQL')"><span>{{d_words.tl_SQL}}</span></div>
                    <div title="Swift " class="dropdown-item" @click.stop="$click_code('Swift')"><span>{{d_words.tl_Swift}}</span></div>
                    <div title="VB.Net " class="dropdown-item" @click.stop="$click_code('vbnet')"><span>{{d_words.tl_VB}}</span></div>
                    <div title="Shell " class="dropdown-item" @click.stop="$click_code('Shell')"><span>{{d_words.tl_Shell}}</span></div>
                    <div title="Objective-C " class="dropdown-item" @click.stop="$click_code('ObjectiveC')"><span>{{d_words.tl_ObjectiveC}}</span></div>
                </div>
            </transition>
        </div>
        ...
        <button :disabled="!editable" type="button" v-if="toolbars.table" @click="$clicks('table')"
                ...
                :title="`${d_words.tl_table} (ctrl+alt+t)`"><span style="display:flex">表格</span></button>
        ...
        <button type="button" v-if="toolbars.undo" @click="$clicks('undo')" class="op-icon fa fa-mavon-undo"
                aria-hidden="true" :title="`${d_words.tl_undo} (ctrl+z)`"><span style="display:flex">上一步</span></button>
        <button type="button" v-if="toolbars.redo" @click="$clicks('redo')" class="op-icon fa fa-mavon-repeat"
                aria-hidden="true"
                :title="`${d_words.tl_redo} (ctrl+y)`"><span style="display:flex">下一步</span></button>
        <button type="button" v-if="toolbars.trash" @click="$clicks('trash')" class="op-icon fa fa-mavon-trash-o"
                aria-hidden="true"
                :title="`${d_words.tl_trash} (ctrl+breakspace)`"><span style="display:flex">清空</span></button>
        <button type="button" v-if="toolbars.save" @click="$clicks('save')" class="op-icon fa fa-mavon-floppy-o"
                aria-hidden="true"
                :title="`${d_words.tl_save} (ctrl+s)`"><span style="display:flex">保存</span></button>
    ...
    export default {
        ...
        data() {
            return {
                ...
                s_code_dropdown_open: false,
                s_coder_dropdown_open: false,
        ...
        methods: {
            ...
            $mouseenter_code_dropdown() {
                if (this.editable) {
                    clearTimeout(this.code_timer)
                    this.s_code_dropdown_open = true
                }
            },
            $mouseleave_code_dropdown() {
                let vm = this
                this.code_timer = setTimeout(function() {
                    vm.s_code_dropdown_open = false
                },200)
            },
            $mouseenter_coder_dropdown() {
                if (this.editable) {
                    clearTimeout(this.coder_timer)
                    this.s_coder_dropdown_open = true
                }
            },
            $mouseleave_coder_dropdown() {
                let vm = this
                this.coder_timer = setTimeout(function() {
                    vm.s_coder_dropdown_open = false
                },200)
            },
            ...
            $click_code(_type) {
                // 让父节点来绑定事件并
                this.$emit('toolbar_left_click', _type);
                this.s_coder_dropdown_open = false
            },
            handleClose(e) {
                this.s_img_dropdown_open = false;
            }
